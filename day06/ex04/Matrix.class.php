...
class Matrix {
    ...
    {
        $val = new Matrix(array('preset' => ""));
        for ($i = 0; $i < 4; $i++) {
            for ($j = 0; $j < 4; $j++) {
                $val->matrix[$i][$j] = $this->matrix[$j][$i];
            }
        }
        return ($val);
    }
    ...
    public function transformVertex($vtx)
    {
        ...
        $revolutions->setX(($vtx->getX() * $this->matrix[0][0]) + ($vtx->getY() * $this->matrix[0][1]) + ($vtx->getZ() * $this->matrix[0][2]) + ($vtx->getW() * $this->matrix[0][3]));
        $revolutions->setY(($vtx->getX() * $this->matrix[1][0]) + ($vtx->getY() * $this->matrix[1][1]) + ($vtx->getZ() * $this->matrix[1][2]) + ($vtx->getW() * $this->matrix[1][3]));
        $revolutions->setZ(($vtx->getX() * $this->matrix[2][0]) + ($vtx->getY() * $this->matrix[2][1]) + ($vtx->getZ() * $this->matrix[2][2]) + ($vtx->getW() * $this->matrix[2][3]));
        $revolutions->setW(($vtx->getX() * $this->matrix[3][0]) + ($vtx->getY() * $this->matrix[3][1]) + ($vtx->getZ() * $this->matrix[3][2]) + ($vtx->getW() * $this->matrix[3][3]));
        return ($revolutions);
    }    
